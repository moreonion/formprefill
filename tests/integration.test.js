...
QUnit.test('Exclusion and inclusion of fields', function (assert) {
  const $excluded = $('<div data-form-prefill-exclude></div>').appendTo(this.$form)
  const $excludedField = $('<input type="text" data-form-prefill-keys="nickname">').appendTo($excluded)
  const $includedField = $('<select data-form-prefill-include name="myform[personal_data][favourite_pet][]"><option value="cat">Cat</option><option value="dog">Dog</option></select>').appendTo($excluded)
...
QUnit.test('Write all values', function (assert) {
  const done = assert.async()
  const api = formPrefill(this.$form[0])
